...
- `{articles_dir}/{article_slug}.json` — full article record
- `{recommendations_dir}/{article_slug}.json` — all 5-7 recommendations
- `{decisions_json}` — user accept/reject toggles (look up `articles.{slug}.recs.{rec_id}`). If no decision recorded, default = accept.
- `{brands_dir}/{peec_project_id}/brand-voice.md` — voice artefact
- `{media_dir}/{article_slug}/*` — downloaded images (to re-embed by local path)
...
1. Re-read `{brands_dir}/{peec_project_id}/brand-voice.md`.
...
Write to `{optimised_dir}/{article_slug}.md`:
...
Write `{optimised_dir}/{article_slug}.html`:
...
- All `<img>` use **local paths** relative to `{optimised_dir}/media/{slug}/` (copied from `{media_dir}/{slug}/`)
...
Write `{optimised_dir}/{article_slug}.handoff.md`:
...
Copy images from `{media_dir}/{slug}/` → `{optimised_dir}/media/{slug}/` so the output folder is self-contained.
...
Human-input items: {K}.
Preview: {optimised_dir}/{slug}.html
Handoff: {optimised_dir}/{slug}.handoff.md
